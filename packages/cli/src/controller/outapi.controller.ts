...
export class APIController {
  ...
  async convertAudio(inputFilePath, outputFilePath) {
    return new Promise((resolve, reject) => {
      ...
      voice.decode(
        inputFilePath, outputFilePath, { format: "mp3" },
        (file) => resolve(file));
    });
  }
  ...
  async audioToText(@Files() files: any) {
    ...
    try {
      ...
      const filePath: any = await this.convertAudio(inputFilePath, this.filePath + '/' + uuidv4() + '.mp3')
      ...
      const data: any = await this.httpService.postForm(this.voiceApi, formData, {
        params: {
          encode: true,
          task: 'transcribe',
  ...
  async imgToText(@Files() files: any) {
    ...
    try {
      ...
      const data: any = await this.httpService.postForm(this.imgTextApi, formData, {
        headers: {
          ...formData.getHeaders(), // 设置请求头为 FormData 对象的头部信息
        },
      })
      const text = get(data, 'data.data.raw_out', []).map(it => it[1]).join('')
