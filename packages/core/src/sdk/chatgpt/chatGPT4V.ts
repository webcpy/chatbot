...
async function sendRequest({
	...
	let response;
	if (debug) {
		log.success(['gpt4v request', url, data, JSON.stringify(headers), params]);
	}
...
export async function getImageVision(images, question, config) {
				...
				content: [
					{
						...
						text:
							question ||
							'Please describe the content of the image in Chinese. If it contains any prohibited content, please refrain from describing it.',
					},
				],
			},
		],
	...
	if (res.status !== 200) {
		if (config.debug) {
			log.success(['gpt4v request error', res.data]);
		}
