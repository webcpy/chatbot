// @ts-nocheck
import { ChatClient } from '@chat-bot/core';
...
class DifyAi {
	...
	async getReply(content, uid, adminId = '', systemMessage = '') {
		try {
			...
			const { conversationId, text, files } = systemMessage
				? await this.difyChat.sendMessage(content, {
						...this.chatOption[uid],
						systemMessage,
						timeoutMs: this.config.timeoutMs * 1000 || 80 * 1000,
						user: uid,
					})
				: await this.difyChat.sendMessage(content, {
						...this.chatOption[uid],
			...
			if (this.config.showQuestion) {
				message =
					`${content}${this.eol}-----------${this.eol}` +
					(this.iswindows ? text.replaceAll('\n', this.eol) : text);
			} else {
			...
			if (!imgs.length && files.length && this.config.showDownloadUrl) {
				...
				replys[replys.length - 1].content =
					`${replys[replys.length - 1].content}${this.eol}${this.eol}${downLoadUrl}`;
			}
			if (imgs.length) {
