import {get4vReply, getDify4vReply} from "../botInstance/gpt4v";
import {getImageVision} from '../proxy/multimodal'
...
class BotManage {
  ...
  maxuser: number;
  userBotDict: any;
  constructor(maxuser = 50, that: any) {
    this.Bot = that;
  ...
  async creatBot(username: any, content: { id: any; }) {
    log.success("bot process create");
  ...
  // 更新对话
  async updateBot(username: string | number, content: { id: any; }, config?: any) {
    log.success(`更新{${username}}对话`);
    this.userTimeDict[username] = new Date().getTime();
    return await this.talk(username, content, config);
  }

  async talk(username: any, content: { id?: any; type?: any; content?: any; }, config: any) {
    if (this.userBotDict[username].step == 0) {
      ...
      log.success("第二轮对话，用户输入了需要提问的内容");
   ...
   * @returns
   */
  async generateImage(username: string | number, question: any, config: { robotType: number; }) {
    const images = [];
    if(config.robotType == 8) {
      ...
      return replys;
    } else if(config.robotType == 11) {
      const length =  this.userBotDict[username].imageIds.length - 1
      const id = this.userBotDict[username].imageIds[length]
      const msg = await this.Bot.Message.find({ id })
      const file = await msg.toFileBox()
      const base = await file.toDataURL()
      images.push(base)
  ...
  getImage(username: string, content: { id: any; }, step: number) {
  ...
  // 对话入口
  async run(userId: string, content: any, config: any) {
    if (content.type === 1) {
