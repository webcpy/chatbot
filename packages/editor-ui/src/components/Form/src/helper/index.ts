 ...
 */
export const setItemComponentSlots = (slotsProps: Recordable = {}, formModel: any): Recordable => {
  const slotObj: Recordable = {}
  for (const key in slotsProps) {
    if (slotsProps[key]) {
      if (isFunction(slotsProps[key])) {
        if (key == 'default') {
          // slotObj[humpToDash(key)] = slotObj[humpToDash(key)](formModel)
          slotObj[humpToDash(key)] = () => {
            // console.log(5, 6, 7, 8, key, slotsProps[key], formModel)
            return slotsProps[key]?.(formModel)
          }
        } else {
          slotObj[humpToDash(key)] = (...args: any[]) => {
            return slotsProps[key]?.(...args, formModel)
          }
        }
      } else {
        slotObj[humpToDash(key)] = () => {
