...
import { getEventKeywords, delEventKeywords } from '@/api/dashboard/workplace'
...
const { tableRegister, tableState, tableMethods } = useTable({
  fetchDataApi: async () => {
    ...
    const res = await getEventKeywords({
  ...
  fetchDelApi: async () => {
    const res = await delEventKeywords(unref(ids))
    return !!res
  }
})
...
const crudSchemas = reactive<CrudSchema[]>([
  ...
  },
  {
    ...
    formatter: (data: any) => {
      return <div>{eventStatus[data.event]}</div>
    }
  },
  {
    field: 'action',
...
const delData = async (row: any) => {
  ...
  try {
    ...
    await delEventKeywords(unref(ids).join(',')).finally(() => {
...
const action = async (row: any, type: string) => {
  if (type == 'edit') {
    push(`/wechat/keywordsEvent/edit/${row.id}`)
  }
}
...
<template>
  <ContentWrap
    title="技能中心
"
    message="当好友发送或群中@机器人 预设关键词时，会自动触发对应技能，部分关键词只可在群里触发，例如群合影
"
  >
    <div class="mb-10px">
