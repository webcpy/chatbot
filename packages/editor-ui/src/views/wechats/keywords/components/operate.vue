...
import { saveKeywords, updateKeywords, detailKeywords } from '@/api/dashboard/workplace'
...
const getTableDet = async () => {
  ...
  const res = await detailKeywords(props.id)
  if (res) {
...
const save = async () => {
  ...
  if (formData) {
    ...
    let res: any = null
    if (props.type == 'edit') {
      res = await updateKeywords(formData)
        .catch(() => {})
        .finally(() => {
          loading.value = false
        })
    } else {
      res = await saveKeywords(formData)
        .catch(() => {})
        .finally(() => {
          loading.value = false
        })
