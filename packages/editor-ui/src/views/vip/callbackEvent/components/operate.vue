...
import { saveCallBackEvents, updateCallBackEvents, detailCallBackEvents } from '@/api/vip'
...
const getTableDet = async () => {
  ...
  const res = await detailCallBackEvents(props.id)
  if (res) {
...
const save = async () => {
  ...
  if (formData) {
    ...
    let res: any = null
    if (props.type == 'edit') {
      res = await updateCallBackEvents(formData)
        .catch(() => {})
        .finally(() => {
          loading.value = false
        })
    } else {
      res = await saveCallBackEvents(formData)
        .catch(() => {})
        .finally(() => {
          loading.value = false
        })
