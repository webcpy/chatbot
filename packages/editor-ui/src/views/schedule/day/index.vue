...
import {
  getDayTaskSchedule,
  delDayTaskSchedule,
  detailDayTaskSchedule,
  updateDayTaskSchedule,
  saveDayTaskSchedule
...
const { tableRegister, tableState, tableMethods } = useTable({
  fetchDataApi: async () => {
    ...
    const res = await getDayTaskSchedule({
...
const crudSchemas = reactive<CrudSchema[]>([
  ...
  },
  {
    ...
    formatter: (data: any) => {
      return data.city && <ElTag type="primary">{data.city}</ElTag>
    }
  },
  {
    field: 'endWord',
...
const delData = async (row: any) => {
  ...
  await delDayTaskSchedule(unref(ids).join(',')).finally(() => {
...
const save = async () => {
  ...
  if (formData) {
    ...
    if (actionType.value == 'edit') {
      res = await updateDayTaskSchedule(
        omit(
          {
            ...formData,
            date,
