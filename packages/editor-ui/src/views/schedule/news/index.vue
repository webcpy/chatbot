...
import {
  getRoomNewsSchedule,
  delRoomNewsSchedule,
  updateRoomNewsSchedule,
  saveRoomNewsSchedule
} from '@/api/schedule/index'
import { useClipboard } from '@/hooks/web/useClipboard'
import { omit, get } from 'lodash-es'
import { newStatus } from './data'
...
const { tableRegister, tableState, tableMethods } = useTable({
  fetchDataApi: async () => {
    ...
    const res = await getRoomNewsSchedule({
...
const crudSchemas = reactive<CrudSchema[]>([
  ...
  },
  {
    ...
    formatter: (data: any) => {
      return data.sortId && <span type="primary">{newStatus[data.sortId]}</span>
    }
  },
  {
    field: 'endWord',
...
const delData = async (row: any) => {
  ...
  await delRoomNewsSchedule(unref(ids).join(',')).finally(() => {
...
const save = async () => {
  ...
  if (formData) {
    ...
    if (actionType.value == 'edit') {
      res = await updateRoomNewsSchedule(
        omit(
          {
            ...formData,
            date,
      ...
      res = await saveRoomNewsSchedule({
        ...formData,
        names,
        date
      })
...
<template>
  <ContentWrap tittle="定时新闻资讯">
    <p
      >申请天行数据的分类新闻接口即可：<a href="https://www.tianapi.com/apiview/51" target="_blank"
        >申请链接</a
      >
    </p>
